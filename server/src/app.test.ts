...
describe('HTTP API',()=>{
 ...
 it('does not leak unexpected errors',async()=>{const{app,token,query}=await fixture();query.mockRejectedValueOnce(new Error('postgresql://admin:secret@internal/sql'));const r=await app.inject({method:'GET',url:'/api/orders',headers:{authorization:`Bearer ${token}`}});expect(r.statusCode).toBe(500);expect(r.body).not.toMatch(/secret|postgresql|admin@/i)})
})
