  ...
  >
    ...
    <nav class="px-4 py-6">
      <ul class="space-y-2">
        ...
        <li data-testid="member-navigation" class="pt-4">
          <p class="px-4 py-2 text-sm font-heading font-semibold text-ink-muted uppercase">會員</p>
          <ul class="space-y-1 ml-2">
            <li>
              <router-link
                to="/profile"
                class="flex items-center gap-3 px-4 py-2 rounded-full text-sm text-ink hover:bg-white/50 transition-colors"
                :class="isActive('/profile') ? 'bg-gradient-to-br from-primary-from to-primary-to text-white shadow-emphasis' : ''"
                @click="$emit('update:open', false)"
              >
                <span
                  class="w-6 h-6 rounded-full flex items-center justify-center text-xs shrink-0"
